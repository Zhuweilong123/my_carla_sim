...
from planner import planner_utiles, motion_plan_path_planning
from controller.Controller import Vehicle_control
from planner.global_path_plan import global_path_planner
...
while True:
    ...
    if count % 500 == 0:  # 这里表示控制器执行一千规划器执行一次
        ...
        if len(possible_obs) != 0 and possible_obs[0][1] <= 30 and possible_obs[-1][
            ...
            obs_s_list, obs_l_list = planner_utiles.cal_s_l_fun(obs_xy, local_frenet_path_opt, s_map)
            ...
            local_frenet_path_opt = motion_plan_path_planning.frenet_2_x_y_theta_kappa(plan_start_s=begin_s_list[0],
                                                                                       plan_start_l=begin_l_list[0],
